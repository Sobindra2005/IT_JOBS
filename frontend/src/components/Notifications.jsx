...
import { seenHandle } from "../api/Notifications";
...
function Notifications() {
  ...
  const seen = async (id) => {
    const responce =await seenHandle(id)
    setData(responce.data)
  };
  return (
    <>
      <div
        className={`Notification  z-9 fixed mt-16 left-0  cursor-pointer h-screen  bg-white  w-1/4`}
      >
        <h2 className="text-xl text-white h-12 font-bold py-3 pl-1 fixed w-1/4  text-center  bg-notification ">
          Notifications
        </h2>
